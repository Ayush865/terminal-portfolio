...
export default function Hero({ onTerminalToggle }) {
  ...
  const playNamePronunciation = () => {
    const utterance = new SpeechSynthesisUtterance("Ayush Prakash");
    // Try to set an Indian English voice if available, otherwise default
    const voices = window.speechSynthesis.getVoices();
    const indianVoice = voices.find(
      (voice) => voice.lang.includes("en-IN") || voice.name.includes("India")
    );
    if (indianVoice) {
        ...
        <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <div className="mb-4 flex flex-col gap-4">
              <div
                className={`text-sm ${textColorClasses[currentMessage.color]
            ...
            <div className="flex gap-6 text-gray-400 mb-12">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                aria-label="View my LeetCode profile"
                className="p-2.5 border border-gray-800 rounded hover:border-green-500/30 hover:text-green-400 transition-all"
                onMouseEnter={(e) => {
                  e.currentTarget.style.boxShadow =
                    "0 0 10px rgba(74, 222, 128, 0.2), 0 0 20px rgba(74, 222, 128, 0.1)";
